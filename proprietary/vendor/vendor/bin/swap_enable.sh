...
function oplus_configure_tuning_swappiness() {
	...
	case "$product_id" in
		"25922"|"25923"|"26921"|"26971") # Himalayan, Himalayan-O, JingGangShan, JingGangShan-O
			echo 1  > $para_path/dynamic_direct_swappiness
			echo 1200  > $para_path/dynamic_direct_swappiness_threshold
			# disable dynamic swappiness
			echo 0  > $para_path/vm_swappiness_threshold1
			echo 0 > $para_path/swappiness_threshold1_size
			echo 0  > $para_path/vm_swappiness_threshold2
			echo 0 > $para_path/swappiness_threshold2_size
...
function configure_memory_parameters() {
	...
	#yala & konka project min_free_kbytes, increase min_free_kbytes from 13556 to 23168
	case "$product_id" in
		"23105"|"23106"|"23216"|"23101"|"23205")
		if [ $MemTotal -gt 8388608 ]; then
