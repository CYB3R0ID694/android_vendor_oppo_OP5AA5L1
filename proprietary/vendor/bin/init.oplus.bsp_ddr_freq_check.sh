...
do
    if [ "$element" = "QCOM" ]; then
        ...
        if  [ $PLAT == "QCOM" ]; then
            ...
            line=$opp$p
            echo "line is $line"
            DVFSRC_OPP_TABLE_LINE=$(cat ${DVFSRC_PATH}dvfsrc_opp_table | sed -n $line | grep -o '[.*[0-9]*[[:space:]]*[kM]bps')
            DDR_FREQ=$(cat ${DVFSRC_PATH}dvfsrc_opp_table | sed -n $line | grep -o '[.*[0-9]*[[:space:]]*[kM]bps' | grep -o '[0-9]*')
...
DDR_FREQ_TEST_TABLE_SORTED=()
for i in $(seq 1 ${#DDR_FREQ_TABLE[*]});
do
  DDR_FREQ_TEST_TABLE_SORTED[i]=0
done
...
do_ddr_freq_check(){
    ...
    do
        ...
        for i in ${DDR_FREQ_TEST_TABLE_SORTED[@]}
        do
            echo "i is $i"
            if [ $i == $element ]; then
    ...
    echo "DDR_FREQ_TEST_TABLE is : ${DDR_FREQ_TEST_TABLE_SORTED[@]}"
    ...
    echo "DDR_FREQ_TEST_TABLE is : ${DDR_FREQ_TEST_TABLE_SORTED[@]}" >> /data/DDR_FREQ_CHECK_RESULT.txt
...
do_get_ddr_freq(){
    ...
    DDR_FREQ_TEST_TABLE_SORTED=($(printf "%s\n" "${DDR_FREQ_TEST_TABLE[@]}" | sort -n))
    echo "DDR_FREQ_TEST_TABLE_SORTED is : ${DDR_FREQ_TEST_TABLE_SORTED[@]}"
